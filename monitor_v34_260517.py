"""
Altcoin Pump Monitor — v3.4-260517 専用 (独立BOT)
GitHub Actions で 1時間ごとに実行

【この BOT について】
2026-05-17 のグリッドサーチで発見された新戦略 "v3.4-260517" 専用の独立 BOT。
...
DISCORD_WEBHOOK = os.environ.get("DISCORD_WEBHOOK_V34_260517", "").strip()
...
# ============= 戦略パラメータ (v3.4-260517) =============
STRATEGY_LABEL = "v3.4-260517"
STRATEGY_NOTE = "案D: 大爆騰特化 / SL=entry+10% / 48h保有 / 2h待ち"
...
STATE_FILE = Path(__file__).parent / "monitor_v34_260517_state.json"
...
def discord_notify(content, embeds=None):
    if not DISCORD_WEBHOOK:
        log("WARN: DISCORD_WEBHOOK_V34_260517 未設定、通知スキップ")
        return False
    ...
    req = urllib.request.Request(DISCORD_WEBHOOK, data=body, headers={
        ...
        "User-Agent": "DiscordBot (v34-260517, 1.0)"
    })
    try:
    ...
    for s in state["scheduled"]:
        ...
        if not s.get("entry_notified"):
            ...
            if mins_until <= ENTRY_WINDOW_MIN:
                ...
                ok = discord_notify(f"🆕 **v3.4-260517 ENTRY [{sym}]** いますぐ発注!", embeds=[embed])
                if ok:
        ...
        if s.get("entry_notified") and not s.get("exit_notified") and not sl_fired_this_run:
            ...
            if mins_until <= EXIT_WINDOW_MIN:
                ...
                ok = discord_notify(f"⏰ **v3.4-260517 決済 [{sym}]** {HOLD_HOURS}h 経過、成行決済を!", embeds=[embed])
                if ok:
...
def main():
    ...
    if not CG_API_KEY:
        ...
        sys.exit(1)
    if not DISCORD_WEBHOOK:
        log("WARN: DISCORD_WEBHOOK_V34_260517 未設定(通知なし)")

    # 接続テスト
    if os.environ.get("TEST_DISCORD_V34_260517", "").strip() == "1":
        ...
        ok = discord_notify(
            ...
            f"確認後、Variable の TEST_DISCORD_V34_260517 を削除してください。"
        )
