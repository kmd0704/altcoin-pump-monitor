"""
Altcoin Pump Monitor — v3.6-260517 専用 (独立BOT)
GitHub Actions で 1時間ごとに実行

【この BOT について】
v3.4-260517 と同フィルタで TP のみ 40% に深く設定した版。
同じpump銘柄を v3.4(TP25%) と v3.6(TP40%) のペアで取りに行く想定。
既存 monitor.py (v3.4-v3.6+s+m版) とは完全に独立して動く。

【戦略仕様】
  検知条件:
    - 24h変動率: 70%〜1000% (注: 既存BOTは 50%〜200%)
    - 時価総額ランク: 100〜1000 (注: 既存BOTは 250〜1000)
    - 30日前比: ≤ 3.0倍 (= ret_30d ≤ 200%)
    - 7日 turnover: ≥ 0.5%/日 (注: 既存BOTは 1%/日)
    - vol_z: ≥ 0 (NaN は通す)
  挙動:
    - エントリー: pump_start + 2時間後 (即時でも3hでもない、中間)
    - 利確 TP: -25% (entry比)
    - 損切 SL: +10% (entry比、Peak×倍率ではない)
    - 保有時間: 48時間
  期待値 (バックテスト):
    - 月間 約 17件
    - pnl>0率 76% / TP-hit率 16% (深いので届きにくい)
    - PF 7.59 / SL-hit率 24% / 時間切れ 60%
    - 平均PnL/件 +15.8%
    - Sharpe 0.87

【環境変数 (GitHub Secrets / Variables)】
  CG_API_KEY:                    CoinGecko API キー(共有 OK)
  DISCORD_WEBHOOK_V36_260517:    Discord webhook URL (★ 新規・専用)
  CG_PLAN:                       "demo" or "pro" (デフォルト: demo)
  ACCOUNT_BALANCE:               口座残高(円、デフォルト: 100000)
  POSITION_PCT:                  ポジションサイズ(0-1、デフォルト: 0.20)
  TEST_DISCORD_V36_260517:       "1" なら接続テストのみ
"""
import os
import sys
import json
...
# ============= 戦略パラメータ (v3.4-260517) =============
STRATEGY_LABEL = "v3.6-260517"
STRATEGY_NOTE = "v3.4と同フィルタ+TP 40%深掘り / SL=entry+10% / 48h保有 / 2h待ち"

# 検知フィルタ
CH24_MIN = 0.70                 # 24h変動率 ≥ 70%
CH24_MAX = 10.00                # 24h変動率 ≤ 1000%
RANK_MIN = 100                  # 時価総額ランク ≥ 100
RANK_MAX = 1000                 # 時価総額ランク ≤ 1000
RATIO_30D_MAX = 3.0             # 30日前比 ≤ 3.0倍 (ret_30d ≤ 200%)
TURNOVER_MIN = 0.005            # 7日 turnover ≥ 0.5%/日
...
STATE_FILE = Path(__file__).parent / "monitor_v36_260517_state.json"
...
def basic_filter(coin):
    """basic_filter: rank / ch24 / ステーブル除外 (v3.4-260517 専用)"""
...
def discord_notify(content, embeds=None):
    if not DISCORD_WEBHOOK:
        log("WARN: DISCORD_WEBHOOK_V36_260517 未設定、通知スキップ")
        return False
    ...
    req = urllib.request.Request(DISCORD_WEBHOOK, data=body, headers={
        ...
        "User-Agent": "DiscordBot (v36-260517, 1.0)"
    })
    try:
...
def build_entry_embed(schedule, entry_price):
    ...
    return {
        ...
        "color": 0xe74c3c,  # 赤(v3.4-260517と区別)
        "fields": [
        ...
        "footer": {"text": f"v3.6-260517 / SL=entry+{int(SL_PCT*100)}% / TP=-{int(TP_PCT*100)}% / hold={HOLD_HOURS}h / wait={WAIT_HOURS}h"},
...
def build_exit_embed(schedule):
    ...
    return {
        ...
        "footer": {"text": f"v3.6-260517 / {HOLD_HOURS}h 経過"}
    }
...
def detect_phase(state):
    """Phase 1: v3.4-260517 条件を満たす銘柄を検出"""
...
def main():
    ...
    if not CG_API_KEY:
        ...
        sys.exit(1)
    if not DISCORD_WEBHOOK:
        log("WARN: DISCORD_WEBHOOK_V36_260517 未設定(通知なし)")

    # 接続テスト
    if os.environ.get("TEST_DISCORD_V36_260517", "").strip() == "1":
        ...
        ok = discord_notify(
            ...
            f"確認後、Variable の TEST_DISCORD_V36_260517 を削除してください。"
        )
